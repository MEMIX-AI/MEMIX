...
//
// `configured` is derived server-side (app/layout.tsx reads
// process.env.ANTHROPIC_API_KEY, a secret that can't be read from this
// client component) — when false, the panel shows a coming-soon state
// instead of a chat box that looks live but 503s on the first message.
export function LibrarianWidget({ configured }: { configured: boolean }) {
          ...
          {!configured ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
              ...
              <p className="text-sm text-dim">
                chat search is on the way — for now, browse{" "}
                <a href="/library" className="font-medium text-accent underline">
                  the library
                </a>{" "}
                directly.
              </p>
